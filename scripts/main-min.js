...
/* main.js */
;(function($){(function(){var $body=$("body"),$buttons,buttonsData=["sunny","rain","cloudy","snow","night","auto"],ajaxCall,preload,$headerOverlay=$('<div class="overlay" />').appendTo("#header").css("opacity","0"),$headerLoader=$('<div class="loader"><div></div></div>').insertAfter($headerOverlay),loaderVisible=false;$(function(){initButtons();if(!$.cookies.get("weather")){$buttons.filter(".weather-auto").click();}else if($.cookies.get("weather_auto")&&$.cookies.get("weather_auto")==="1"){$buttons.removeClass("active").filter(".weather-auto").addClass("active");}});function initButtons(){var buttons=[];$("#sidebar > .weather label").each(function(i){var $this=$(this);buttons[i]=$('<button type="button" class="'+$this.attr("for")+" "
+$this.attr("class")+'" value="'+$this.attr("for").slice(6)+'" title="'
+$this.text()+'"></button>').insertAfter(this)[0];});$buttons=$(buttons);$buttons=$buttons.add($('<p><button type="button" class="weather-auto" value="auto" title="Auto"></button></p>').insertAfter($buttons.filter(":last").closest("p")).children());$buttons.click(function(){if(!!ajaxCall){ajaxCall.abort();}
var curMeteo=$(this).attr("value");$.cookies.del("weather");$.cookies.del("weather_auto");$buttons.removeClass("active");$(this).addClass("active");if(curMeteo==="auto"){autoChangeMeteo();}else{changeMeteo(curMeteo);}});};function autoChangeMeteo(){showLoading();ajaxCall=$.ajax({url:$.lesintegristes.themeUrl+"/weather_service/service.php",success:function(data){hideLoading();changeMeteo(data,{hoursToLive:1});$.cookies.set("weather_auto","1",{hoursToLive:1});},cache:false});};function changeMeteo(weather,settings){if(loaderVisible){hideLoading();}
settings=$.extend({hoursToLive:24},settings);updateBodyClass(weather);$.cookies.set("weather",weather,settings);};function updateBodyClass(weather){$body.removeClass("weather-"+buttonsData.join(" weather-")).addClass("weather-"+weather);}
