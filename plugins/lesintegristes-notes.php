...
function lesintegristes_notes_title_intercept($postID, $post) {
	...
	//$cur_post_id = ( wp_is_post_revision($postID) )? $post->post_parent : $postID;
	//$parent_post = get_post($cur_post_id);
	
	if (isset($_POST["post_category"]) && in_array("31", $_POST["post_category"])) {
		
		$post_excerpt   = substr(strip_tags($post->post_content), 0, 50);
